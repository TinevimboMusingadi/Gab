...
func snapshotDir(scope string, excludes []string, st *storage.Store) (models.State, error) {
	...
	err := filepath.WalkDir(scope, func(path string, d fs.DirEntry, err error) error {
		...
		art.ComputeID()
		if err := st.PutObject(art.ID, &art); err != nil {
			return err
		}
	...
	s.ComputeID()
	if err := st.PutObject(s.ID, &s); err != nil {
		return models.State{}, err
	}
	return s, nil
}
...
func (r *Recorder) RecordCommand(ctx context.Context, in RecordCommandInput) (*RecordedEvent, error) {
	...
	_ = ev.ComputeID()
	if err := r.store.PutObject(ev.ID, &ev); err != nil {
		return nil, err
	}
	if err := r.idx.SetHead(in.AgentID, ev.ID); err != nil {
		return nil, err
	}
	if err := r.idx.AddEventTime(in.AgentID, ev.ID, finish); err != nil {
		return nil, err
	}
