...
// View provides access to the underlying BadgerDB for custom queries
func (i *Index) View(fn func(*badger.Txn) error) error {
	return i.db.View(fn)
}
...
func (i *Index) ListByTime(agentID string, limit int) ([]TimeEntry, error) {
	...
	err := i.db.View(func(txn *badger.Txn) error {
		...
		for it.Rewind(); it.ValidForPrefix(prefix); it.Next() {
			...
			hash := k[idx+1:]
			var nsec int64
			if err := item.Value(func(v []byte) error { return json.Unmarshal(v, &nsec) }); err != nil {
				return err
			}
